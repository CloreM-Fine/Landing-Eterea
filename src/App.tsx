...
function CustomCursor() {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 w-4 h-4 rounded-full bg-eterea-dark pointer-events-none z-[9999] mix-blend-difference hidden md:block"
        animate={{
          x: position.x - 8,
          y: position.y - 8,
      ...
      />
      <motion.div
        className="fixed top-0 left-0 w-8 h-8 rounded-full border border-eterea-dark/30 pointer-events-none z-[9998] hidden md:block"
        animate={{
          x: position.x - 16,
          y: position.y - 16,
        }}
