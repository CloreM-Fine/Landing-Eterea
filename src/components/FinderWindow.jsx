...
// Componente Cartella (visualizzazione griglia)
function MacOSFolder({ name, color, colors, onClick }) {
  ...
  return (
    ...
    >
      <div className="relative mb-2 w-20 h-[68px]">
      ...
      <span className="text-xs text-center font-medium px-1 py-0.5 rounded" style={{ color: colors.folderLabel }}>
        {name}
      </span>
    </motion.div>
  );
...
// Componente Cartella (visualizzazione lista)
function MacOSFolderList({ name, color, colors, onClick }) {
  const isPurple = color === '#8b5cf6';
  const mainColor = color;
  const tabColor = isPurple ? '#a78bfa' : '#a5e0fd';
  const uniqueId = `${name.replace(/\s+/g, '-').toLowerCase()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  return (
    ...
    >
      ...
      <span className="text-sm font-medium truncate" style={{ color: colors.finderText }}>
        {name}
      </span>
    </motion.div>
  );
...
export function FinderWindow({ colors, isFinderLight, onClose, zIndex = 50, onFocus, onFolderClick, onOpenMaps, onOpenMedia }) {
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ backgroundColor: colors.finderBg }}>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b" style={{ backgroundColor: colors.finderHeaderBg, borderColor: colors.finderBorder, height: '50px' }}>
            <div className="flex items-center gap-3">
              <span className="font-semibold text-sm select-none" style={{ color: colors.finderText }}>Eterea</span>
            </div>
            <div className="flex items-center gap-1">
              ...
              <button 
                ...
                className={`p-1.5 rounded transition-colors ${colorFilterActive ? 'bg-blue-500/20' : 'hover:bg-black/10'}`}
                    ...
                    {purpleFolders.length > 0 && (
                      <div className="grid grid-cols-4 gap-4">
                        {purpleFolders.map((folder) => (
                          <MacOSFolder key={folder.id} name={folder.name} color={folder.color} colors={colors} onClick={onFolderClick} />
                        ))}
                      </div>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    {purpleFolders.length > 0 && (
                      <div className="space-y-0.5">
                        <div className="px-3 py-1 text-xs font-semibold uppercase tracking-wider opacity-50" style={{ color: colors.finderText }}>
                          Eterea
                        </div>
                        {purpleFolders.map((folder) => (
                          <MacOSFolderList key={folder.id} name={folder.name} color={folder.color} colors={colors} onClick={onFolderClick} />
                        ))}
                      </div>
                    )}
                    ...
                    {/* Separatore */}
                    {purpleFolders.length > 0 && blueFolders.length > 0 && (
                      <div className="w-full h-px my-2" style={{ backgroundColor: colors.finderBorder }} />
                    )}
